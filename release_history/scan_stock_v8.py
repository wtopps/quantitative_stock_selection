...
class StockScreener:
    ...
    def calculate_composite_score(self, fund_consistency, fund_flow_ratio, rs_score, position_score, original_signal_strength):
        ...
        """
        # 各维度权重 - v8.0优化：提高资金权重
        weight_fund = 0.45      # 资金流向权重（从35%提升至45%）
        weight_rs = 0.25        # 相对强度权重（从30%降至25%）
        weight_position = 0.20  # 价格位置权重（从25%降至20%）
    ...
    def step11_multidimensional_analysis(self, df):
        """
        第十一步：三维度综合分析（v8.1优化版）
        整合：资金共振 + 市场相对强度 + 关键价格位置
        v8.1新增：综合评分阈值筛选 + 风险收益比筛选 + 板块信息获取
        """
        print("\n" + "-" * 50)
        print("【第十一步】🎯 三维度综合分析（v8.1优化版）")
        print("   📊 维度1: 资金共振（主力+整体一致性）")
        print("   📈 维度2: 市场相对强度（跑赢大盘）")
        print("   📍 维度3: 关键价格位置（突破+支撑）")
        print("   🔥 v8.1新增: 综合评分≥55 + 风险收益比≥1.5 + 板块信息")
        ...
        if not df_result.empty:
            ...
            print(f"\n   ✅ 多维度分析完成: {len(df_result)} 只 (已过滤: 综合评分≥55 & 风险收益比≥1.5)")
            print(f"   🏆 综合评级: AAA={aaa_count} | AA={aa_count} | A={a_count}")
            print(f"   📈 相对强势: {strong_rs} 只跑赢大盘")
            print(f"   📍 位置良好: {good_position} 只处于有利位置")
            print(f"   🔥 v8.1: 龙头={leader_count}只(超级龙头={super_leader}) | 风险收益比≥2={good_rr}只(≥3={excellent_rr}只)")
...
def main():
    ...
    print("  🎯 最终筛选: 综合评分≥55 & 风险收益比≥1.5 & 最多20只")
    print("  🚫 板块限制: 仅沪深主板（已排除创业板、科创板、北交所）")
    print("  💰 权重调整: 资金权重45% | 20日胜率 | 历史记录功能")
