...
class StockScreener:
    ...
    def print_header(self):
        """打印头部信息"""
        print("=" * 70)
        print("【A股次日冲高标的筛选系统 v4.0】")
        print(f"筛选日期: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("🆕 新增功能: 超大单/大单资金流向监控")
        print("=" * 70)
    ...
    def get_stock_fund_flow_hist(self, stock_code, days=5):
        """
        获取个股历史资金流向数据（近N日）
        用于判断资金流向的持续性
        """
        try:
            # 获取个股历史资金流向
            df = ak.stock_individual_fund_flow(stock=stock_code, market="sh-sz")
            if df is not None and not df.empty:
                # 取最近N天数据
